...
class FullContextAgent:
    ...
    def __init__(self, max_turns: int = 400, max_context_tokens: int = 8192):
    ...
    def _estimate_tokens(self, text: str) -> int:
        # 对中文和英文混合文本做一个保守的近似估算，避免超过模型上下文上限。
    ...
    def ingest(self, conversation: dict) -> None:
        ...
        # 先按轮数做一个宽松上限，再按 token 预算从后往前截断。
        if len(lines) > self.max_turns:
            lines = lines[-self.max_turns:]

        budget = max(1024, self.max_context_tokens - 1024)
        selected = []
        used_tokens = 0
        for line in reversed(lines):
            ...
            if not selected and line_tokens > budget:
                # 单行过长时也要保留，至少确保不会因为空 prompt 直接失效。
